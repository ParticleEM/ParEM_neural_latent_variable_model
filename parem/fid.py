...
def compute_fid(samples, dataset, n_samples=100):
    ...
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        mu_data, sigma_data = calculate_activation_statistics(dataset, sess, batch_size=100)
        mu_samples, sigma_samples = calculate_activation_statistics(samples, sess, batch_size=100)

    fid_value = calculate_frechet_distance(mu_data, sigma_data, mu_samples, sigma_samples)
    return fid_value
